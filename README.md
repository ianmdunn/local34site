...
## Ghost headless CMS + Astro frontend

This site uses **Ghost as a headless CMS** (admin + Content API only) and **Astro as the sole public frontend**. Ghost does not serve the public site; Astro fetches blog content from the Ghost Content API at build time (or at runtime in dev) and renders all pages.

- **Required environment variables**: `GHOST_CONTENT_API_URL` and `GHOST_CONTENT_API_KEY`. Copy [.env.example](.env.example) to `.env` and set these (get the key from Ghost Admin: Settings → Integrations → Add custom integration).
- **Docker**: Run `docker compose up -d --build` to build and start Astro (port 8080), Ghost (port 2368), and MySQL. Astro waits for Ghost to be healthy before starting. Ghost admin: http://localhost:2368/ghost
- **Production**: Run `npm run build` with Ghost env vars set (e.g. in CI). For Docker production build, use the Dockerfile `production` target with `--build-arg GHOST_CONTENT_API_URL=...` and `--build-arg GHOST_CONTENT_API_KEY=...`; Ghost must be reachable at build time.
- **Editing pages in Ghost**: To move all Astro site pages (home, about, actions, etc.) into Ghost so you can edit them in Ghost Admin, see [docs/GHOST-PAGES-MIGRATION.md](docs/GHOST-PAGES-MIGRATION.md). Run `npm run export:ghost-pages` to generate a Ghost import file, then import it in Ghost (Settings → Labs → Import content). The site prefers Ghost pages over local markdown when both exist.
- **Docker shows AstroWind demo footer?** When the homepage uses Ghost (e.g. in Docker), the **main content area** is the HTML of Ghost’s “home” page. If that page was imported from an AstroWind demo or contains full-page HTML (including the Product/Platform/Support/Company footer), that block appears in the middle of the page. The site’s real footer is from `src/components/widgets/Footer.astro` and `src/navigation.ts`. To remove the demo footer: open **Ghost Admin** (http://localhost:2368/ghost), edit the **home** page, and delete the demo footer (and any other demo content) from the page body so only your intended content remains.
...
Set `GHOST_CONTENT_API_URL` and `GHOST_CONTENT_API_KEY` (see [.env.example](.env.example)) so the blog can load content from Ghost.
...
For **dev.local34.org**, set **`SITE_URL=https://dev.local34.org`** in your build environment and configure your server/host for that domain. See [docs/deploy-dev.md](docs/deploy-dev.md).
